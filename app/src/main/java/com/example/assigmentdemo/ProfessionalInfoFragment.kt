...
class ProfessionalInfoFragment : Fragment() {
    ...
    override fun onResume() {
        super.onResume()
        val arrayAdapter=ArrayAdapter<EducationEnum>(requireContext(),R.layout.drop_down,EducationEnum.values())
...
fun check():Boolean{
  ...
  education= binding.autoComplete.text.toString()
    ...
    {
        return Validation.errormsg(requireContext(),"Enter your year of passing")
    }
    if (!Validation.isCharactValid(binding.universityEdt))
    {
        return Validation.errormsg(requireContext(),"Enter your university")
    }
    if (!Validation.isCharactValid(binding.gradeEdt))
    {
        return Validation.errormsg(requireContext(),"Enter your grade")
    }
    if (!Validation.isCharactValid(binding.yoeEdt))
    {
        return Validation.errormsg(requireContext(),"Enter year of experience")
    }
    if (!Validation.isCharactValid(binding.discriptionEdt))
    {
        return Validation.errormsg(requireContext(),"Enter designation")
    }
    if (!Validation.isCharactValid(binding.domainEdt))
    {
        return Validation.errormsg(requireContext(),"Enter your domain")
    }
    return true
